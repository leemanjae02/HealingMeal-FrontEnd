...
const FindPWPage = () => {
  ...
  const FindPW = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!name || !email || !id) {
      setMessage("이름과 이메일 아이디를 모두 입력해주세요.");
      return;
    } else {
      setMessage("");
    }
    try {
      const response = await CustomAxios.get(
        `/user/search/pwd/${encodeURIComponent(name)}/${encodeURIComponent(
          email
        )}`
      );
      // 요청 성공 시 처리, {
      ...
      setMessage("이름과 이메일 아이디를 다시 확인하고 입력해주세요.");
      console.log(error);
    }
  };
  ...
  return (
    <div className={styles.Container}>
      ...
      <div className={styles.Box}>
        ...
        <button className={styles.next_Btn} type="submit" onClick={FindPW}>
          요청
        </button>
      </div>
      <Footer />
